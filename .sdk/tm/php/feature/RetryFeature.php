...
// FuelPricesAtSpanishGasStations SDK retry feature
...
class FuelPricesAtSpanishGasStationsRetryFeature extends FuelPricesAtSpanishGasStationsBaseFeature
{
    ...
    public function init(FuelPricesAtSpanishGasStationsContext $ctx, array $options): void
    {
        ...
        $utility->fetcher = function (FuelPricesAtSpanishGasStationsContext $ctx2, string $url, array $fetchdef) use ($inner): array {
            return $this->_with_retry($ctx2, $url, $fetchdef, $inner);
        };
    }

    private function _with_retry(FuelPricesAtSpanishGasStationsContext $ctx, string $url, array $fetchdef, callable $inner): array
    {
        $opts = $this->options ?? [];
    ...
    private function _track(FuelPricesAtSpanishGasStationsContext $_ctx, int $attempt, mixed $res, mixed $err, float $wait): void
    {
        $client = $this->client;
