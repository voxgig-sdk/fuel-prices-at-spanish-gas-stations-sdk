...
// FuelPricesAtSpanishGasStations SDK feature test
//
...
class FtCtrl extends FuelPricesAtSpanishGasStationsControl
{
...
class FtClient
{
    ...
    private FuelPricesAtSpanishGasStationsUtility $utility;

    public function __construct(FuelPricesAtSpanishGasStationsUtility $utility, array $options)
    {
    ...
    public function get_utility(): FuelPricesAtSpanishGasStationsUtility
    {
        return $this->utility;
    }
}
...
class FtRecorder
{
    ...
    {
        return function (FuelPricesAtSpanishGasStationsContext $_ctx, string $url, array $fetchdef): array {
...
class FtHarness
{
    ...
    public FuelPricesAtSpanishGasStationsContext $rootctx;
    ...
    public function __construct(
        ...
        $utility = new FuelPricesAtSpanishGasStationsUtility();
        $this->utility = $utility;

        $utility->fetcher = $server ?? function (FuelPricesAtSpanishGasStationsContext $_ctx, string $_url, array $fetchdef): array {
        ...
        foreach ($features as $fspec) {
            ...
            $f = FuelPricesAtSpanishGasStationsConfig::make_feature($name);
    ...
    private static function buildUrl(FuelPricesAtSpanishGasStationsSpec $spec): string
    {
        $keys = [];
    ...
    public function op(array $o = []): array
    {
        ...
        if (($ctx->out['point'] ?? null) instanceof FuelPricesAtSpanishGasStationsError) {
        ...
        if (is_array($fetched)) {
            $ctx->response = new FuelPricesAtSpanishGasStationsResponse($fetched);
        }
    ...
    private function fail(FuelPricesAtSpanishGasStationsContext $ctx, mixed $err): array
    {
...
class FeatureTest extends TestCase
{
    ...
    public function test_retry_retries_a_transport_error_until_exhausted(): void
    {
        ...
        $clock = new FtClock();
        $n = 0;
        $server = function (FuelPricesAtSpanishGasStationsContext $ctx, string $_url, array $_fd) use (&$n): array {
            $n++;
