...
// FuelPricesAtSpanishGasStations SDK utility: make_point

require_once __DIR__ . '/../core/Helpers.php';

class FuelPricesAtSpanishGasStationsMakePoint
{
    public static function call(FuelPricesAtSpanishGasStationsContext $ctx): array
    {
        if (isset($ctx->out['point'])) {
            ...
            if ($ctx->out['point'] instanceof FuelPricesAtSpanishGasStationsError) {
        ...
        if (count($op->points) === 1) {
            ...
            foreach ($op->points as $p) {
                $point = $p;
                $select_def = FuelPricesAtSpanishGasStationsHelpers::to_map(\Voxgig\Struct\Struct::getprop($p, 'select'));
                $found = true;
            ...
            if ($reqselector) {
                $req_action = \Voxgig\Struct\Struct::getprop($reqselector, '$action');
                if ($req_action && $point) {
                    $point_select = FuelPricesAtSpanishGasStationsHelpers::to_map(\Voxgig\Struct\Struct::getprop($point, 'select'));
