# fuel-prices-at-spanish-gas-stations-cli

boru-driven command-line client **and** interactive REPL for the FuelPricesAtSpanishGasStations
...
```sh
# 1. Build a native binary (-> dist/<os>-<arch>/fuel-prices-at-spanish-gas-stations-cli)
make build

# 2. See usage (words, entities, env vars)
./fuel-prices-at-spanish-gas-stations-cli --help

# 3. Provide credentials once, via the environment
export FUEL_PRICES_AT_SPANISH_GAS_STATIONS_APIKEY=sk_live_xxx

# 4. Each command line is ONE boru expression, run against the API:
./fuel-prices-at-spanish-gas-stations-cli load 1 dataset            # {id:1} shorthand
./fuel-prices-at-spanish-gas-stations-cli load '{id:1}' dataset       # explicit match map

# 5. Override the API base URL for a single call
FUEL_PRICES_AT_SPANISH_GAS_STATIONS_BASE=https://api.example.com ./fuel-prices-at-spanish-gas-stations-cli load 1 dataset

# 6. No arguments -> interactive REPL
./fuel-prices-at-spanish-gas-stations-cli
fuel-prices-at-spanish-gas-stations> load 1 dataset
fuel-prices-at-spanish-gas-stations> /quit
```
...
1. **Build the binary.** From this `go-cli/` directory:
   ...
   ```sh
   make build          # -> dist/<os>-<arch>/fuel-prices-at-spanish-gas-stations-cli
   ```
...
3. **Run a query.** Evaluate an boru expression against the API (or run with no
   ...
   ```sh
   ./dist/*/fuel-prices-at-spanish-gas-stations-cli load 1 dataset
   ```
...
```sh
./fuel-prices-at-spanish-gas-stations-cli load 1 dataset          # scalar shorthand for {id:1}
./fuel-prices-at-spanish-gas-stations-cli load '{id:1}' dataset     # explicit match map
```
...
./fuel-prices-at-spanish-gas-stations-cli load 1 dataset
```
...
Run with no arguments to open a REPL (prompt `fuel-prices-at-spanish-gas-stations>`). Each line is
evaluated as its own boru expression:

```text
$ ./fuel-prices-at-spanish-gas-stations-cli
fuel-prices-at-spanish-gas-stations> load 1 dataset
fuel-prices-at-spanish-gas-stations> /help
fuel-prices-at-spanish-gas-stations> /quit
```
...
| Target | Result |
|--------|--------|
| `make build` | Native binary at `dist/<os>-<arch>/fuel-prices-at-spanish-gas-stations-cli`. |
...
(`./fuel-prices-at-spanish-gas-stations-cli <expr>`) and interactively (the REPL), and expressions compose the
