...
[MCP](https://modelcontextprotocol.io) server exposing the FuelPricesAtSpanishGasStations SDK as
two agent tools — `fuel-prices-at-spanish-gas-stations_list` and `fuel-prices-at-spanish-gas-stations_load` — built on the
...
```sh
# 1. Build a native binary (-> dist/<os>-<arch>/fuel-prices-at-spanish-gas-stations-mcp)
make build

# 2. Provide credentials via the environment
export FUEL_PRICES_AT_SPANISH_GAS_STATIONS_APIKEY=sk_live_xxx
...
```jsonc
// fuel-prices-at-spanish-gas-stations_list: first page of records
{ "entity": "dataset" }
{ "entity": "dataset", "query": { } }

// fuel-prices-at-spanish-gas-stations_load: one record by id
...
1. **Build** the server from this `go-mcp/` directory:
   ...
   ```sh
   make build          # -> dist/<os>-<arch>/fuel-prices-at-spanish-gas-stations-mcp
   ```
...
3. **Install it into Claude Code** (stdio transport):
   ...
   ```sh
   claude mcp add --scope user fuel-prices-at-spanish-gas-stations \
     -- "$PWD"/dist/*/fuel-prices-at-spanish-gas-stations-mcp -transport stdio
   ```

4. **Restart Claude Code.** The `fuel-prices-at-spanish-gas-stations_list` and `fuel-prices-at-spanish-gas-stations_load` tools now appear
   in new sessions. Ask the agent to *"list dataset using fuel-prices-at-spanish-gas-stations"*
   and it calls `fuel-prices-at-spanish-gas-stations_list` with `{"entity":"dataset"}`.
...
```sh
export FUEL_PRICES_AT_SPANISH_GAS_STATIONS_APIKEY=sk_live_xxx            # API key
export FUEL_PRICES_AT_SPANISH_GAS_STATIONS_BASE=https://api.example.com  # optional: override the API base URL
```
...
### Call the `fuel-prices-at-spanish-gas-stations_list` tool
...
### Call the `fuel-prices-at-spanish-gas-stations_load` tool
...
| Tool | Args | Returns |
|------|------|---------|
| `fuel-prices-at-spanish-gas-stations_list` | `entity` (required), `query` (optional map) | First page of records as JSON |
| `fuel-prices-at-spanish-gas-stations_load` | `entity` (required), `query` = `{id:N}` | Single record as JSON |
...
| Variable | Purpose |
|----------|---------|
| `FUEL_PRICES_AT_SPANISH_GAS_STATIONS_APIKEY` | API key sent with every request. |
| `FUEL_PRICES_AT_SPANISH_GAS_STATIONS_BASE` | Optional override of the API base URL. |
