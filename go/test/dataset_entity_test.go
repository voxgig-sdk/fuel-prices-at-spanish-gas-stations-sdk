...
func TestDatasetEntity(t *testing.T) {
	...
	t.Run("basic", func(t *testing.T) {
		...
		if setup.syntheticOnly {
			t.Skip("live entity test uses synthetic IDs from fixture — set FUELPRICESATSPANISHGASSTATIONS_TEST_DATASET_ENTID JSON to run live")
			return
		}
		client := setup.client
...
func datasetBasicSetup(extra map[string]any) *entityTestSetup {
	...
	entidEnvRaw := os.Getenv("FUELPRICESATSPANISHGASSTATIONS_TEST_DATASET_ENTID")
	...
	idmapResolved := core.ToMapAny(env["FUELPRICESATSPANISHGASSTATIONS_TEST_DATASET_ENTID"])
