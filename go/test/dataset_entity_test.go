...
func TestDatasetEntity(t *testing.T) {
	...
	t.Run("basic", func(t *testing.T) {
		...
		if setup.syntheticOnly {
			t.Skip("live entity test uses synthetic IDs from fixture — set FUEL_PRICES_AT_SPANISH_GAS_STATIONS_TEST_DATASET_ENTID JSON to run live")
			return
		}
		client := setup.client
...
func datasetBasicSetup(extra map[string]any) *entityTestSetup {
	...
	entidEnvRaw := os.Getenv("FUEL_PRICES_AT_SPANISH_GAS_STATIONS_TEST_DATASET_ENTID")
	...
	idmapResolved := core.ToMapAny(env["FUEL_PRICES_AT_SPANISH_GAS_STATIONS_TEST_DATASET_ENTID"])
