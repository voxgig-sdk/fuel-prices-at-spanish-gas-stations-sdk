...
import (
	...
	omni "github.com/voxgig-sdk/fuel-prices-at-spanish-gas-stations-sdk/go/test/omni"
...
func omniProvider(client any) *omni.Provider {
	provider := &omni.Provider{
		...
		Errify: func(err any) any {
			if sdkerr, is := err.(*sdk.FuelPricesAtSpanishGasStationsError); is {
				out := map[string]any{
					"name":    "FuelPricesAtSpanishGasStationsError",
...
func omniCtx(arg any, client *sdk.FuelPricesAtSpanishGasStationsSDK, utility *sdk.Utility) *sdk.Context {
	...
	if provider, is := ctxmap["client"].(*omni.Provider); is {
		if live, is := defproviderclient(provider).(*sdk.FuelPricesAtSpanishGasStationsSDK); is {
			client = live
