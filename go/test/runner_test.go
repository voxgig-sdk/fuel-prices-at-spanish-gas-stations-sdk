...
func envOverride(m map[string]any) map[string]any {
	if os.Getenv("FUEL_PRICES_AT_SPANISH_GAS_STATIONS_TEST_LIVE") == "TRUE" ||
		os.Getenv("FUEL_PRICES_AT_SPANISH_GAS_STATIONS_TEST_OVERRIDE") == "TRUE" {
		for key := range m {
	...
	if explain := os.Getenv("FUEL_PRICES_AT_SPANISH_GAS_STATIONS_TEST_EXPLAIN"); explain != "" {
...
func makeCtxFromMap(ctxmap map[string]any, client *sdk.FuelPricesAtSpanishGasStationsSDK, utility *sdk.Utility) *sdk.Context {
	if ctxmap == nil {
	...
	if resMap, ok := ctxmap["result"].(map[string]any); ok {
		...
		if errMap, ok := resMap["err"].(map[string]any); ok {
			if msg, ok := errMap["message"].(string); ok {
				ctx.Result.Err = &sdk.FuelPricesAtSpanishGasStationsError{Msg: msg}
			}
		}
	}
...
func fixctx(ctx *sdk.Context, client *sdk.FuelPricesAtSpanishGasStationsSDK) {
...
func errFromMap(m map[string]any) error {
	...
	return &sdk.FuelPricesAtSpanishGasStationsError{Msg: msg, Code: code}
}
