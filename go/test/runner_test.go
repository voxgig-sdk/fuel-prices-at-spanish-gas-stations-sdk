...
func envOverride(m map[string]any) map[string]any {
	if os.Getenv("FUELPRICESATSPANISHGASSTATIONS_TEST_LIVE") == "TRUE" ||
		os.Getenv("FUELPRICESATSPANISHGASSTATIONS_TEST_OVERRIDE") == "TRUE" {
		for key := range m {
	...
	if explain := os.Getenv("FUELPRICESATSPANISHGASSTATIONS_TEST_EXPLAIN"); explain != "" {
