...
local DistributionEntity = {}
DistributionEntity.__index = DistributionEntity


function DistributionEntity.new(client, entopts)
  entopts = entopts or {}
  ...
  local self = setmetatable({}, DistributionEntity)
...
function DistributionEntity:get_name()
  return self._name
end


function DistributionEntity:make()
  local opts = {}
  for k, v in pairs(self._entopts) do
    opts[k] = v
  end
  return DistributionEntity.new(self._client, opts)
end
...
function DistributionEntity:mark_deleted()
  self._deleted = true
end


function DistributionEntity:deleted()
  return true == self._deleted
end


function DistributionEntity:data_set(args)
  if args ~= nil then
...
function DistributionEntity:data_get()
  self._utility.feature_hook(self._entctx, "GetData")
  return vs.clone(self._data)
end


function DistributionEntity:match_set(args)
  if args ~= nil then
...
function DistributionEntity:match_get()
...
function DistributionEntity:stream(action, args, callopts)
...
---@param reqmatch DistributionLoadMatch
---@param ctrl? table
---@return Distribution
---@return string? err
function DistributionEntity:load(reqmatch, ctrl)
  local utility = self._utility
...
function DistributionEntity:_run_op(ctx, post_done)
  local utility = self._utility
