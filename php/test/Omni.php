...
//    only - a typed FuelPricesAtSpanishGasStationsContext there would make every ctx
...
final class FuelPricesAtSpanishGasStationsOmni
{
    ...
    public static function ctx_from_map($ctxmap, $client, $utility)
    {
        ...
        $ctx = new FuelPricesAtSpanishGasStationsContext($clean, null);
        ...
        if (isset($clean['spec']) && is_array($clean['spec'])) {
            $ctx->spec = new FuelPricesAtSpanishGasStationsSpec($clean['spec']);
        }

        if (isset($clean['result']) && is_array($clean['result'])) {
            $ctx->result = new FuelPricesAtSpanishGasStationsResult($clean['result']);
            if (isset($clean['result']['err']) && is_array($clean['result']['err'])) {
                $msg = $clean['result']['err']['message'] ?? '';
                $ctx->result->err = new FuelPricesAtSpanishGasStationsError('', is_string($msg) ? $msg : '');
            }
        }

        if (isset($clean['response']) && is_array($clean['response'])) {
            $ctx->response = new FuelPricesAtSpanishGasStationsResponse($clean['response']);
    ...
    public static function err_from_map($m): ?FuelPricesAtSpanishGasStationsError
    {
        if (!is_array($m)) {
            return null;
        }
        ...
        $code = $m['code'] ?? '';
        return new FuelPricesAtSpanishGasStationsError(is_string($code) ? $code : '', $msg);
    }
