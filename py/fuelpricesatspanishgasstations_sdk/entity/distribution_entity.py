...
from fuelpricesatspanishgasstations_sdk.fuelpricesatspanishgasstations_types import (
    Distribution,
...
class DistributionEntity:
    ...
    def make(self):
        ...
        for k, v in self._entopts.items():
            opts[k] = v
        return DistributionEntity(self._client, opts)
    ...
    def data_get(self) -> Distribution:
    ...
    def match_get(self) -> Distribution:
    ...
    def load(self, reqmatch=None, ctrl=None) -> Distribution:
        utility = self._utility
        # reqmatch is optional: an entity with no id-like key loads with no
        # match. Treat None as an empty match so client.Distribution().load()
        # works with no args.
        if reqmatch is None:
            reqmatch = {}
