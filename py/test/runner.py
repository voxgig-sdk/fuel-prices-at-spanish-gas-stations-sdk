# FuelPricesAtSpanishGasStations SDK test runner
...
class FuelPricesAtSpanishGasStationsTestRunner:
    _env = {}
    ...
    def load_env_local():
        ...
        for line in content.splitlines():
            ...
            FuelPricesAtSpanishGasStationsTestRunner._env[key] = val

    @staticmethod
    def getenv(key):
        val = FuelPricesAtSpanishGasStationsTestRunner._env.get(key)
        if val is not None:
            return val
    ...
    @staticmethod
    def env_override(m):
        live = FuelPricesAtSpanishGasStationsTestRunner.getenv("FUEL_PRICES_AT_SPANISH_GAS_STATIONS_TEST_LIVE")
        override = FuelPricesAtSpanishGasStationsTestRunner.getenv("FUEL_PRICES_AT_SPANISH_GAS_STATIONS_TEST_OVERRIDE")

        if live == "TRUE" or override == "TRUE":
            for key in list(m.keys()):
                envval = FuelPricesAtSpanishGasStationsTestRunner.getenv(key)
        ...
        explain = FuelPricesAtSpanishGasStationsTestRunner.getenv("FUEL_PRICES_AT_SPANISH_GAS_STATIONS_TEST_EXPLAIN")
    ...
    def load_test_control():
        ...
        """
        if FuelPricesAtSpanishGasStationsTestRunner._test_control is not None:
            return FuelPricesAtSpanishGasStationsTestRunner._test_control
        ctrl_path = os.path.join(os.path.dirname(__file__), "sdk-test-control.json")
        try:
            with open(ctrl_path, "r") as f:
                FuelPricesAtSpanishGasStationsTestRunner._test_control = json.load(f)
        except (FileNotFoundError, IOError, ValueError):
            FuelPricesAtSpanishGasStationsTestRunner._test_control = {
                "version": 1,
                "test": {"skip": {
    ...
    def is_control_skipped(kind, name, mode):
        ...
        ctrl = FuelPricesAtSpanishGasStationsTestRunner.load_test_control()
...
def load_env_local():
    FuelPricesAtSpanishGasStationsTestRunner.load_env_local()


def env_override(m):
    return FuelPricesAtSpanishGasStationsTestRunner.env_override(m)


def entity_data(v):
    return FuelPricesAtSpanishGasStationsTestRunner.entity_data(v)


def entity_list_to_data(lst):
    return FuelPricesAtSpanishGasStationsTestRunner.entity_list_to_data(lst)


def is_control_skipped(kind, name, mode):
    return FuelPricesAtSpanishGasStationsTestRunner.is_control_skipped(kind, name, mode)


def load_test_control():
    return FuelPricesAtSpanishGasStationsTestRunner.load_test_control()


def live_client_options():
    return FuelPricesAtSpanishGasStationsTestRunner.live_client_options()


def live_delay_ms():
    return FuelPricesAtSpanishGasStationsTestRunner.live_delay_ms()
