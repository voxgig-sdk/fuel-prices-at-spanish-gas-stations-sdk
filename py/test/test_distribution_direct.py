...
class TestDistributionDirect:

    def test_should_direct_load_distribution(self):
        setup = _distribution_direct_setup({"id": "direct01"})
        _skip, _reason = runner.is_control_skipped("direct", "direct-load-distribution", "live" if setup["live"] else "unit")
        if _skip:
...
def _distribution_direct_setup(mockres):
    ...
    if live:
        ...
        client = FuelPricesAtSpanishGasStationsSDK(merged_opts)
        return {
            "client": client,
            "calls": calls,
            "live": True,
