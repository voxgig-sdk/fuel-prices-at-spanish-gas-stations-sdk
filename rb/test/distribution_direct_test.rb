...
class DistributionDirectTest < Minitest::Test
  def test_direct_load_distribution
    setup = distribution_direct_setup({ "id" => "direct01" })
    _should_skip, _reason = Runner.is_control_skipped("direct", "direct-load-distribution", setup[:live] ? "live" : "unit")
    if _should_skip
...
def distribution_direct_setup(mockres)
