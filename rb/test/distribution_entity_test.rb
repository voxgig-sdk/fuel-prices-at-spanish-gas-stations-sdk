...
class DistributionEntityTest < Minitest::Test
  ...
  def test_basic_flow
    setup = distribution_basic_setup(nil)
    # Per-op sdk-test-control.json skip.
    _live = setup[:live] || false
    ["load"].each do |_op|
      _should_skip, _reason = Runner.is_control_skipped("entityOp", "distribution." + _op, _live ? "live" : "unit")
      if _should_skip
    ...
    if setup[:synthetic_only]
      skip "live entity test uses synthetic IDs from fixture — set FUEL_PRICES_AT_SPANISH_GAS_STATIONS_TEST_DISTRIBUTION_ENTID JSON to run live"
      return
    end
    client = setup[:client]
    ...
    distribution_ref01_data = nil
    if distribution_ref01_data_raw.length > 0
...
def distribution_basic_setup(extra)
  Runner.load_env_local

  entity_data_file = File.join(__dir__, "..", "..", ".sdk", "test", "entity", "distribution", "DistributionTestData.json")
  ...
  entid_env_raw = ENV["FUEL_PRICES_AT_SPANISH_GAS_STATIONS_TEST_DISTRIBUTION_ENTID"]
  ...
  if env["FUEL_PRICES_AT_SPANISH_GAS_STATIONS_TEST_LIVE"] == "TRUE"
    ...
    client = FuelPricesAtSpanishGasStationsSDK.new(Helpers.to_map(merged_opts))
  end
