...
module FuelPricesAtSpanishGasStationsUtilities
  MakeRequest = ->(ctx) {
    ...
    spec = ctx.spec
    utility = ctx.utility
    response = FuelPricesAtSpanishGasStationsResponse.new({})
    ...
    if fetch_err
      ...
      response = FuelPricesAtSpanishGasStationsResponse.new({ "err" => ctx.make_error("request_no_response", "response: undefined") })
    elsif fetched.is_a?(Hash)
      response = FuelPricesAtSpanishGasStationsResponse.new(fetched)
    else
