...
describe('DistributionDirect', async () => {
  ...
  test('direct-exists', async () => {
    const sdk = new FuelPricesAtSpanishGasStationsSDK({
  ...
  test('direct-load-distribution', async (t: any) => {
    if (liveScenariosActive()) { t.skip('Covered by live operation scenarios'); return }
    const setup = directSetup({ id: 'direct01' })
    if (maybeSkipControl(t, 'direct', 'direct-load-distribution', setup.live)) return
...
function directSetup(mockres?: any) {
  ...
  if (live) {
    ...
    let idmap: any = env['FUEL_PRICES_AT_SPANISH_GAS_STATIONS_TEST_DISTRIBUTION_ENTID']
