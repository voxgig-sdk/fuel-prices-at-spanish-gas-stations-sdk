...
//     (\`new FuelPricesAtSpanishGasStationsSDK(...)\` / \`FuelPricesAtSpanishGasStationsSDK.test(...)\` become
//     \`FuelPricesAtSpanishGasStationsSDK.test({ entity: ... })\`), transpiled to JS, and run
...
const SDK_NAME = 'FuelPricesAtSpanishGasStationsSDK'
...
const TEST_SEED = {"entity":{"dataset":{"test01":{"id":"test01"}},"distribution":{"test01":{"id":"test01"}}}}
...
// rewrite real client construction (\`new FuelPricesAtSpanishGasStationsSDK(...)\` and any
// \`FuelPricesAtSpanishGasStationsSDK.test(...)\`) to a seeded test-mode client, and give
...
function rewriteForRun(code: string): string {
  ...
  out = out.replace(/new\s+FuelPricesAtSpanishGasStationsSDK\s*\([^)]*\)/g, () => SEEDED_CTOR)
  out = out.replace(/FuelPricesAtSpanishGasStationsSDK\.test\s*\([^)]*\)/g, () => SEEDED_CTOR)
